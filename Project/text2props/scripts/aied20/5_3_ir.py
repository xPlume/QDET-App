...
from text2props.text2props.modules.feature_engineering.components import IRFeaturesComponent
...
print("[INFO] Start of 5_3_ir.py - ablation study")
...
for min_df in np.arange(0.00, 0.11, 0.02):
    for max_df in np.arange(0.90, 1.01, 0.02):
        ...
        file = open("outputs/5_3_ir_mindf_%.2f_maxdf_%.2f.txt" % (min_df, max_df), 'w')
        ...
        pipe_b = FeatureEngAndRegressionPipeline(
            FeatureEngineeringModule([IRFeaturesComponent(vec_b, concatenate_correct=True, concatenate_wrong=True)]),
            RegressionModule([
        ...
        pipe_a = FeatureEngAndRegressionPipeline(
            FeatureEngineeringModule([IRFeaturesComponent(vec_a, concatenate_correct=True, concatenate_wrong=True)]),
            RegressionModule([
