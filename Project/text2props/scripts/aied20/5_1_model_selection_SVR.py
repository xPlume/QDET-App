from text2props.text2props.constants import (
    DATA_PATH,
    DIFFICULTY,
    DISCRIMINATION,
...
from text2props.text2props.modules.feature_engineering.utils import vectorizer_text_preprocessor as preproc
from text2props.text2props.modules.feature_engineering.components import (
...
print("[INFO] Start of 5_1_model_selection_SVR.py")  # this might take a long time
...
for min_df in np.arange(0.00, 0.11, 0.02):
    for max_df in np.arange(0.90, 1.01, 0.02):
        ...
        file = open("outputs/5_1_model_selection_SVR_mindf_%.2f_maxdf_%.2f.txt" % (min_df, max_df), 'w')
        ...
        pipe_b = FeatureEngAndRegressionPipeline(
            ...
            RegressionModule([SklearnRegressionComponent(SVR(), latent_trait_range=B_RANGE)])
        )
        ...
        pipe_a = FeatureEngAndRegressionPipeline(
            ...
            RegressionModule([SklearnRegressionComponent(SVR(), latent_trait_range=A_RANGE)])
        )
        ...
        dict_params = {
            DIFFICULTY: [{
                'regressor__kernel': ['linear', 'poly', 'rbf'], 'regressor__gamma': ['auto', 'scale'],
                'regressor__shrinking': [True, False], 'regressor__degree': [1, 2, 3, 4]
            }],
            DISCRIMINATION: [{
                'regressor__kernel': ['linear', 'poly', 'rbf'], 'regressor__gamma': ['auto', 'scale'],
                'regressor__shrinking': [True, False], 'regressor__degree': [1, 2, 3, 4]
        ...
        file.write("\nERROR METRICS ON VAL:\n" + str(model.compute_error_metrics_latent_traits_estimation(df_val)))
