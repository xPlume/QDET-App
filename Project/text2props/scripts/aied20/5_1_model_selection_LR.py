...
print("[INFO] Start of 5_1_model_selection_LR.py")
...
for min_df in np.arange(0.00, 0.11, 0.02):
    for max_df in np.arange(0.90, 1.01, 0.02):
        ...
        pipe_a = FeatureEngAndRegressionPipeline(
            ...
            RegressionModule([
                SklearnRegressionComponent(LinearRegression(), latent_trait_range=A_RANGE)
            ])
        )
        ...
        dict_params = {
            DIFFICULTY: [{'regressor__normalize': [True, False]}],
            DISCRIMINATION: [{'regressor__normalize': [True, False]}],
        }
